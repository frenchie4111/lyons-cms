...
?>
	...
	<script src="http://hodenfield.mikelyons.org/admin/menu/add.js"></script>
<?php 
$back = "index.php";
...
<div id='main_container'>
	...
	<?php
		if( isset( $_POST["id"] ) && isset( $_POST["target"] ) && isset( $_POST["title"] ) ) {
			echo $_POST["target"].'<br/>';
			if( $_POST["target"] == "direct" ) {
				if( isset( $_POST["link"] ) ) {
					echo $_POST["link"];
					$query = "UPDATE menu SET title='".mysqli_real_escape_string( $con, $_POST["title"] )."',pageid=0,link='" . mysqli_real_escape_string( $con, $_POST["link"] ) ."' WHERE id=".$_POST['id']."";
					mysqli_query( $con, $query );
					log_action( $con, "Menu Item Edited", "Menu Item edited " . print_r( $_POST, true ) );
				}
			} else {
				$query = "UPDATE menu SET title='".mysqli_real_escape_string( $con, $_POST["title"] )."', pageid=" . mysqli_real_escape_string( $con, $_POST["target"] ) .",link=NULL WHERE id=".$_POST['id'];
				mysqli_query( $con, $query );
				log_action( $con, "Menu Item Edited", "Menu Item edited " . print_r( $_POST, true ) );
			}
	...
	<?php
	if( isset( $_GET["id"] ) ) {
		$row = mysqli_fetch_array( mysqli_query( $con, "SELECT id, title, pageid,link FROM menu WHERE id=" . $_GET['id'] ) );
?>
